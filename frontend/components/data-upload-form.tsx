    ...
    const onSubmit = async (e: React.SyntheticEvent) => {
      ...
      try {
        for (const historyFile of historyFiles) {
          const fileInput = Object.values(historyFile)[0] as HTMLInputElement;
          if (fileInput?.files && fileInput.files.length > 0) {
            const file = fileInput.files![0]
            await handleFileProcessing(file, Object.keys(historyFile)[0]);
          }
          else {
            console.log("No file uploaded for", Object.keys(historyFile)[0]);
          }
        }
